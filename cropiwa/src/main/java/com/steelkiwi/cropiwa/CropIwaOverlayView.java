...
class CropIwaOverlayView extends View implements ConfigChangeListener, OnImagePositionedListener {
    ...
    public CropIwaOverlayView(Context context, CropIwaOverlayConfig config, boolean isSquare) {
        super(context);
        initWith(config);
...
//        float px = 213 * density;
//
//        float viewWidth = px, viewHeight = px;
...
//        }
        float density = getContext().getResources().getDisplayMetrics().density;
        float px = (isSquare ? 288 : 213) * density;
