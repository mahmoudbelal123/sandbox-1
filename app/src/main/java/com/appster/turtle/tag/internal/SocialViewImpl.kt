package com.appster.turtle.tag.internal
...
import com.appster.turtle.tag.SocialView
import com.appster.turtle.tag.SocialView.Companion.REGEX_HASHTAG
import com.appster.turtle.tag.SocialView.Companion.REGEX_HYPERLINK
import com.appster.turtle.tag.SocialView.Companion.REGEX_MENTION
...
class SocialViewImpl<T : TextView> : SocialView<T> {
    ...
    fun setCustomFont() {
        val typeface: Typeface = Typeface.createFromAsset(this.view.context.assets, this.view.context.getString(R.string.eina_01_regular))
    ...
    override var tagList: ArrayList<User>
        get() = userList
        set(value) {
            userList = value
            colorize()
...
//        spannable.setSpan(( AlignmentSpan.Standard(Layout.Alignment.ALIGN_OPPOSITE)), 0, view.text.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);


        if (abc.isNotEmpty()) {
            var index = TextUtils.indexOf(view.text.toString().toLowerCase(), abc.toLowerCase())
            while (index >= 0) {
                ...
                index = TextUtils.indexOf(view.text.toString().toLowerCase(), abc.toLowerCase(), index + abc.length)
            }
        }
        ...
        if (isMentionEnabled)
            spannable.spanMention(REGEX_MENTION, userList, { s ->
